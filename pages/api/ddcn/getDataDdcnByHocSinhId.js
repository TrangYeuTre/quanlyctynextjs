...
const handler = async (req, res) => {
  ...
  const { ngayThongKe, hocSinhId } = body;
  const isValidDataSubmit = () => {
    return ngayThongKe && hocSinhId && ngayThongKe !== "" && hocSinhId !== "";
  };
  if (!isValidDataSubmit()) {
    return res.status(422).json({
      thongbao:
        "Ngày thống kê hoặc học sinh id không hợp lệ để lấy data ngày điểm danh cá nhân.",
    });
  }
